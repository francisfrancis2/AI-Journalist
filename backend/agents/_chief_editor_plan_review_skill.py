"""
Chief Editor Plan Review Skill.
...
class PlanReviewSkill:
    """
    Chief Editor skill that turns storyline feedback into scriptwriter guidance.

    Example::

        skill = PlanReviewSkill()
    ...
    def __init__(self) -> None:
        ...
        self._structured_llm = _llm.with_structured_output(PlanReviewOutput)

    async def run(self, state: dict) -> dict:
        storyline: StorylineProposal | None = state.get("selected_storyline")
        if storyline is None:
            raise ValueError(
                "chief editor plan review received no storyline; chapter writer likely failed upstream"
            )
        ...
        log.info("chief_editor.plan_review.start", topic=topic, title=storyline.title)
        ...
        prompt = (
            ...
            f"{duration_prompt_block(duration_target, role='Chief Editor')}"
        ...
        output: PlanReviewOutput = await self._structured_llm.ainvoke([
        ...
        log.info(
            "chief_editor.plan_review.complete",
            topic=topic,
