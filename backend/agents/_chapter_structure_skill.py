"""
Chapter Structure Skill — embedded inside ChapterWriterAgent.
...
class ChapterStructureSkill:
    """
    Documentary structure skill that builds multi-act storyline proposals.

    Example::

        skill = ChapterStructureSkill()
    ...
    async def run(self, state: dict) -> dict:
        ...
        if analysis is None:
            raise ValueError("chapter_writer structure skill received no analysis_result")
        ...
        log.info("chapter_writer.structure.start", topic=topic, refinement_cycle=refinement_cycle)
            ...
            if isinstance(chapters_data, list):
                ...
                for chapter in chapters_data:
                    ...
                    formatted_chapters.append(
                        "\n".join(
                            [
                                f"- Chapter {chapter.get('chapter_number', '?')}: {chapter.get('title', 'Untitled')}",
                                f"  Purpose: {chapter.get('purpose', '')}",
                                "  Key points: " + "; ".join(str(point) for point in key_points[:6]),
                            ]
                        )
                    )
        ...
        messages = [SystemMessage(content=load_prompt(self.prompt_name)), HumanMessage(content=prompt)]
        ...
        for attempt in range(3):
            try:
                ...
                if result and result.proposals:
                    output = result
                    break
                log.warning("chapter_writer.structure.empty_response", attempt=attempt)
            except (ValidationError, ValueError, TypeError) as exc:
                last_exc = exc
                log.warning("chapter_writer.structure.retry", attempt=attempt, error=str(exc))
                try:
                    recovered = await self._invoke_fallback_json(messages)
                    if recovered.proposals:
                        output = recovered
                        log.info("chapter_writer.structure.recovered_with_json_fallback", attempt=attempt)
                        break
                except Exception as fallback_exc:
                    last_exc = fallback_exc
                    log.warning(
                        "chapter_writer.structure.json_fallback_failed",
                        attempt=attempt,
        ...
        if output is None:
            log.error(
                "chapter_writer.structure.using_deterministic_fallback",
                topic=topic,
        ...
        log.info(
            "chapter_writer.structure.complete",
            topic=topic,
