...
_TOPIC_WORD_RE = re.compile(r"\S+")
_MAX_TOPIC_WORDS = 200
...
class StoryStatus(str, Enum):
    ...
    ANGLE_SELECTION_EXPIRED = "angle_selection_expired"
...
class ImprovementPlan(BaseModel):
    """
    Targeted improvement plan generated by the quality gate after a failed cycle.
    Injected into state so Research, Angles & Hooks, Scriptwriter, and Chief Editor audit
    can direct their work toward closing specific gaps.
    """
    cycle_number: int
    previous_score: float
    research_gaps: list[str] = Field(default_factory=list)
    analysis_focus: list[str] = Field(default_factory=list)
    script_directives: list[str] = Field(default_factory=list)
...
class StoryORM(Base):
    ...
    # Editorial ideation workspace (pre-script planning)
    ideation_stage: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
...
class StoryCreate(BaseModel):
    ...
    topic: str = Field(
        ...,
        min_length=10,
        description="Research topic or question (maximum 200 words)",
    )
