...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const backendResponse = await fetch(`${BACKEND}/api/v1/research/sessions`, {
      method: "POST",
    ...
    return new NextResponse(responseBody, {
      ...
      headers: {
        ...
        "x-ai-journalist-proxy": "research-sessions-create",
      },
    });
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      return jsonError(
        "Research is taking longer than expected. Please try a narrower request.",
        504
      );
    }
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return new NextResponse(responseBody, {
      ...
      headers: {
        "content-type": contentType,
        "x-ai-journalist-proxy": "research-sessions-list",
      },
    });
  } catch {
    return jsonError("Could not connect to the research service. Please try again.", 502);
